CQ5 WITH MVC AND INTEGRATION TESTS WIHT EXTERNAL SLING (MODULES)
...
by page "/system/sling/junit/" or by unit tests. This project
is divided into modules because tests should be run after
installing package with tests on CQ.

Deployment project on author
===========================
mvn clean install crx:install -Pcomponents

Deployment intg tests on author
===============================
mvn clean install crx:install -Pintg-tests-deploy
Here you can run tests by "/system/sling/junit/". This profile
contains also profile "components"

Run intg tests on author
========================
mvn clean install crx:install -Pintg-tests-run
Here tests are run by unit test class. This profile
contains also profile "components" and "ntg-tests-deploy".
